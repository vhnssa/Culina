...
// mengambil hasil kategori healthy food
$selected_kategori = $_GET['namakategori'] ?? 'Healthy Food';

$kategori_result = $conn->query("SELECT DISTINCT namakategori FROM resep");

// siapin query buat ngambil semua data resep berdasarkan kategori
...
<div class="text-start ps-3">
  <h2 class="category-title">Categories</h2>
    <button class="category-button">Healthy Food</button>
</div>
...
<div class="px-4">
  ...
  <div class="recipe-scroll">
    ...
    <?php while ($row = $recipes_result->fetch_assoc()): ?>
      <!-- selama masih ada baris di hasil query tersebut maka akan di tampilkan (loop) -->
      <a href="<?= ($row['namaresep'])?>.php">
      <div class="col-md-4">
        <div class="recipe-card">
          <img src="images/<?= htmlspecialchars($row['foto']) ?>" alt="<?= htmlspecialchars($row['namaresep']) ?>">
          <h5><?= htmlspecialchars($row['namaresep']) ?></h5> 
          <!-- nama resep dijadiin nama file php buat dilink ke resep tersebut -->
        </div>
      </div>
    <?php endwhile; ?>
  </div>
