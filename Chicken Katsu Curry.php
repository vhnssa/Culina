...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add'])) {
    ...
    // Check how many recipes already exist on the selected day
    ...
    if ($checkResult['count'] < 3) {
        ...
        $_SESSION['message'] = "Maximum 3 recipes allowed on $hari.";
    }

    header("Location: Summer Salad.php?idresep=$idresep");
    exit();
}

// Prepare dropdown: Only days with less than 3 recipes
$alldays = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'];
$availabledays = [];
$result = $conn->query("SELECT hari, COUNT(*) as jumlah FROM mealplan GROUP BY hari");
$usedCount = [];
while ($row = $result->fetch_assoc()) {
    $usedCount[$row['hari']] = $row['jumlah'];
}
foreach ($alldays as $day) {
    if (!isset($usedCount[$day]) || $usedCount[$day] < 3) {
        $availabledays[] = $day;
    }
}
?>
...
<div class="hero">
  ...
  <form method="POST" style="position: absolute; top: 130px; right: 100px; display: flex; align-items: center; gap: 10px; z-index: 2;">
